...
// --- NO STEP DO ZUMBI ---
// 2. LÓGICA DE MORTE DO ZUMBI (Deve vir PRIMEIRO de tudo no Step)
if (hp <= 0) 
{
    if (estado != "morto") 
    {
        ...
        mask_index = -1;     // Remove a caixa de colisão (faz a bala atravessar)
        solid = false;       
        image_speed = 1;
...
switch (estado) 
{
	...
	case "PATRULHANDO":
	    ...
	    if (abs(x - destino_x) < 5) {
	        if (destino_x == ponto_b) destino_x = ponto_a;
	        else destino_x = ponto_b;
	    }
